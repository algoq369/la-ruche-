...
export class Blockchain {
    constructor(difficulty = 3, miningReward = 50, storagePath) {
    ...
    addTransaction(tx) {
        ...
        if (!tx.from || !tx.to)
            throw new Error('tx must include from and to');
        ...
        if (!tx.isCoinbase()) {
            ...
            if (spendable < tx.amount + (tx.fee || 0))
                throw new Error('insufficient balance');
            // Nonce must equal next expected
            const expected = this.getNonce(tx.from);
            if (tx.nonce !== expected)
                throw new Error(`invalid nonce: expected ${expected}, got ${tx.nonce}`);
        }
    ...
    isValid() {
        ...
        for (let i = 1; i < this.chain.length; i++) {
            ...
            if (curr.previousHash !== prev.hash)
                return false;
            if (curr.merkleRoot !== curr.computeMerkleRoot())
                return false;
            if (curr.hash !== curr.computeHash())
                return false;
            if (!curr.hash.startsWith('0'.repeat(this.difficulty)))
                return false;
    ...
    _historicalNonceBeforeBlock(address, blockIndex) {
        let n = 0;
    ...
    loadFromDisk() {
        ...
        this.pending = obj.pending.map((t) => {
            const tx = new Transaction(t.from, t.to, t.amount);
            tx.timestamp = t.timestamp;
            tx.signature = t.signature;
            tx.publicKeyPem = t.publicKeyPem;
