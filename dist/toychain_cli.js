...
function getDataPath(args) {
    return resolve(process.cwd(), args['data'] || 'toychain.data.json');
}
function peersFromArgs(args) {
    const csv = args['peers'] || '';
    return csv ? csv.split(',').map((s) => s.trim()).filter(Boolean) : [];
}
async function broadcast(peers, message) {
    await Promise.all(peers.map((url) => new Promise((res) => {
        try {
...
async function main() {
    ...
    switch (cmd) {
        ...
        case 'add-tx': {
            ...
            const walletFile = args['wallet'] || 'wallet.json';
            ...
            if (peers.length)
                await broadcast(peers, { type: 'tx', tx });
            break;
        }
        case 'mine': {
            ...
            const peers = peersFromArgs(args);
            if (peers.length)
                await broadcast(peers, { type: 'block', block });
            break;
        }
        case 'balance': {
